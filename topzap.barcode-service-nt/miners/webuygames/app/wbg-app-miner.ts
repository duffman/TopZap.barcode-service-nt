...
export class WbgAppMiner extends MinerCore {
	...
	public processQueue(dataItems: MinerQueueData[]): Promise<void> {
		...
		function weBuyGamesSearch(dataItem: MinerQueueData): Promise<WbgAppResult> {
		...
		async function process(): Promise<void> {
			...
			for (let i = 0; i < dataItems.length; i++) {
				...
				if (searchResult === null || searchResult.status != 'OK') {
					...
					updateItem.price = Number(searchResult.data.Item.itemPrice);
					updateItem.message = searchResult.data.Item.itemName;
				}
