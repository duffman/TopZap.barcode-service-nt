...
const VENDOR_ID = Vendors.ZiffitApp;

export class ZiffitAppMiner extends MinerCore {
	...
	public processQueue(dataItems: MinerQueueData[]): Promise<void> {
		...
		async function process(): Promise<void> {
			...
			for (let i = 0; i < dataItems.length; i++) {
				...
				if (searchResult === null || searchResult.response_code === 2) {
					...
					updateItem.price = searchResult.product.offer_price; // Risky business here, ok for now...
					updateItem.message = searchResult.product.title;
				}
