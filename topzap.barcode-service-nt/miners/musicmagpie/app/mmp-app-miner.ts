...
const VENDOR_ID = Vendors.WeBuyGamesApp;

export class MmpAppMiner extends MinerCore {
	worker: MmpAppWorker;
	...
	public processQueue(dataItems: MinerQueueData[]): Promise<void> {
		...
		function musicMagpieSearch(dataItem: MinerQueueData): Promise<MagpieResult> {
		...
		async function process(): Promise<void> {
			...
			for (let i = 0; i < dataItems.length; i++) {
				...
				let searchResult = await musicMagpieSearch(item);
				...
				if (searchResult === null || searchResult.GetValuationApps2Result.status === -1) {
					...
					updateItem.price = searchResult.GetValuationApps2Result.price;
					updateItem.message = searchResult.GetValuationApps2Result.description;
				}
