...
export class MmpWebMiner extends MinerCore {
	...
	public processQueue(dataItems: MinerQueueData[]): Promise<void> {
		...
		function musicMagpieSearch(dataItem: MinerQueueData): Promise<Item> {
			Logger.logGreen("Getting price for barcode from somewhere...");

			return new Promise((resolve, reject) => {
				scope.worker.search(dataItem.barcode).then((res) => {
					resolve(res);
				}).catch((err) => {
