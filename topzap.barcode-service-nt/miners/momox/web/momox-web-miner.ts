...
import { MomoxWebWorker}           from "./momox-web-worker";
...
const VENDOR_ID = Vendors.MomoxWeb;

export class StarterMiner extends MinerCore {
	worker: MomoxWebWorker;

	constructor() {
		super();
		this.worker = new MomoxWebWorker();
	}
	...
	public processQueue(dataItems: MinerQueueData[]): Promise<void> {
		...
		function momoxSearch(dataItem: MinerQueueData): Promise<MomoxResult> {
		...
		async function process(): Promise<void> {
			...
			for (let i = 0; i < dataItems.length; i++) {
				...
				let searchResult = await momoxSearch(item);
				...
				if (searchResult === null) {
					...
					updateItem.price = Number(searchResult.price); // Risky business here, ok for now...
